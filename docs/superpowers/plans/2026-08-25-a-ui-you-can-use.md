...
**The page-size budget was 500 KB of HTML, set here rather than after seeing the number, and was RAISED TO 750 KB on 2026-08-27 by the plan owner.** The original number and its reasoning stand below as written, because it did the job it was set to do: it was chosen before anyone measured, it predicted the overrun, it named abbreviated chips as the remedy, and it sent the decision back here when abbreviated chips were not enough. What the measurement then showed is that the limit was not sized for the registry rather than for the markup. At 808 bytes a row with no markers at all, 500 KB runs out at **619 endpoints**; the registry is 543 today and the spec still has YummyData's list and front-end resolution to add, both of which only add rows. So 500 KB was going to break on growth alone, and dormancy's per-row marker merely arrived first. 750 KB holds all 543 in every measured shape, including the pathological one where every row is both marked and dormant, and holds about 900 endpoints in the ordinary shape. **What is still not built is any test: the budget has never been enforced by anything but a reader, which is how a 0.5% overrun survived a commit.** The form to pin is bytes per row rather than a total, because that is the invariant that survives the registry growing.
